...
def login_view(request):
    ...
    if form.is_valid():
        ...
        login(request, user)
        return redirect(reverse('quiz:user_home'))
...
def error_404(request, exception):
    data = {}
    return render(request, 'quiz/error_404.html', data, status=404)

def error_500(request):
    data = {}
    return render(request, 'quiz/error_500.html', data, status=500)